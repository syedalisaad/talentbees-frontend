...
import AdBanner from "@/src/lib/AdBanner";
...
export default function JobsClientContent({ params }: { params: Promise<{ slug?: string[] }> }) {
 ...
 useEffect(() => {
  // 1. Extract the actual string from the slug array
  // If slug is ['software-engineer'], currentSlug becomes 'software-engineer'
  const currentSlug = Array.isArray(slug) ? slug[0] : slug;

  if (currentSlug && jobs.length > 0) {
    // 2. Now you are comparing string === string
    const jobFromSlug = jobs.find((job: Job) => job.slug === currentSlug);
    
    if (jobFromSlug) {
      setSelectedJob(jobFromSlug);
    }
  } else if (!currentSlug && jobs.length > 0) {
    // Default to the first job if no slug is present in the URL
